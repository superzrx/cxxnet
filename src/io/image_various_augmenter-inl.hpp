#ifndef IMAGE_VARIOUS_AUGMENTER_OPENCV_HPP_
#define IMAGE_VARIOUS_AUGMENTER_OPENCV_HPP_
/*!
 * \file image_various_augmenter_opencv.hpp
 * \brief add more image augment
 * \author Ruixin Zhang
 */
...
class ImageVariousAugmenter:public ImageAugmenter {
 public:
  // contructor
   ImageVariousAugmenter(void)
      : tmpres(false),
        rotateM(2, 3, CV_32F) {
    rand_crop_ = 0;
    crop_y_start_ = -1;
  ...
  virtual void SetParam(const char *name, const char *val) {
    ...
    if (!strcmp(name, "bounding_box")) {
      int old_size = bounding_box_list_.size();
      bounding_box_list_.resize(old_size+ 1);
      utils::Check(sscanf(val, "%u,%u,%u,%u", &bounding_box_list_[old_size].x, &bounding_box_list_[old_size].y,
        &bounding_box_list_[old_size].width, &bounding_box_list_[old_size].height) == 4,
        "bounding_box must be four consecutive integers without space example: 0,0,250,250");
    }
    if (!strcmp(name, "pre_size")) pre_size_width_=pre_size_height_=atoi(val);
    if (!strcmp(name, "pre_size_width")) pre_size_width_ = atoi(val);
    if (!strcmp(name, "pre_size_height")) pre_size_height_ = atoi(val);
  ...
  */
  virtual void Process(const cv::Mat &src, std::vector<cv::Mat>& ress,
    ...
    cv::Rect temp_rect(0, 0, temp.cols, temp.rows);
    for (size_t i = 0; i < bounding_box_list_.size(); i++){
      utils::Check((bounding_box_list_[i] & temp_rect) == bounding_box_list_[i], "bounding box exceeds image size");
      cv::Mat res = temp(bounding_box_list_[i]);
      if (max_crop_size_ != -1 || min_crop_size_ != -1){
        ...
        if (pre_size_width_ == -1 || pre_size_height_ == -1){
          ...
          cv::resize(res(roi), res, cv::Size(pre_size_width_,pre_size_height_));
        }
      }
      else{
        int shape_1, shape_2;
        if (pre_size_width_ == -1 || pre_size_height_ == -1){
          ...
          shape_1 = pre_size_width_;
          shape_2 = pre_size_height_;
        }
        utils::Check(static_cast<mshadow::index_t>(res.cols) >= shape_1 && static_cast<mshadow::index_t>(res.rows) >= shape_2,
          "input image size smaller than input shape");
        mshadow::index_t y = res.rows - shape_2;
        mshadow::index_t x = res.cols - shape_1;
        if (rand_crop_ != 0) {
          y = prnd->NextUInt32(y + 1);
          x = prnd->NextUInt32(x + 1);
        }
        ...
        cv::Rect roi(x, y, shape_1, shape_2);
        res = res(roi);
      }
      ress.push_back(res);
    }
 ...
 private:
  ...
  int fill_value_;
  /*! \brief size for resize*/
  int pre_size_width_;
  int pre_size_height_;
  /*! \brief list of possible rotate angle */
  std::vector<int> rotate_list_;
  std::vector<cv::Rect> bounding_box_list_;
};
}  // namespace cxxnet
#endif
