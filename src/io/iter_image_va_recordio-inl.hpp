...
iterator
 */
#ifndef ITER_IMAGE_VA_RECORDIO_INL_HPP_
#define ITER_IMAGE_VA_RECORDIO_INL_HPP_
#include <cstdlib>
#include <dmlc/base.h>
#include <dmlc/io.h>
#include <dmlc/omp.h>
...
class ImageRecordIOParser {
 ...
 private:
  ...
  int nthread_;
  /*! \brief augmenters */
  std::vector<ImageVariousAugmenter*> augmenters_;
...
ParseNext(std::vector<InstVector> *out_vec) {
  ...
  #pragma omp parallel num_threads(nthread_)
  {
    ...
    while (reader.NextRecord(&blob)) {
      ...
      augmenters_[tid]->Process(res,ress,prnds_[tid]);
...
#endif
#endif  // ITER_IMAGE_VA_RECORDIO_INL_HPP_
